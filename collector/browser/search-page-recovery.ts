...
export async function restoreSearchPage(page:Page,targetPage:number,emit:Emit,maxAttempts=3){
  for(let attempt=1;attempt<=maxAttempts;attempt++){
    ...
    emit({type:"pagination",stage:"pagination_restore_start",target:targetPage,current:state.current,attempt,message:`pagination_restore_start target=${targetPage} current=${state.current??"unknown"}`});
    ...
    try{
      for(let moves=0;moves<100&&state.current!==targetPage;moves++){
        ...
        do{await page.waitForLoadState("domcontentloaded").catch(()=>undefined);state=await snapshot(page);if(state.current!==before||state.current===action.target)break;await page.waitForTimeout(200)}while(Date.now()<deadline);
        if(state.current===before&&state.current!==action.target)throw new Error(`PAGINATION_DID_NOT_MOVE from=${before??"unknown"} target=${action.target}`);
      }
      ...
      emit({type:"pagination",stage:"pagination_restore_success",target:targetPage,actual:state.current,message:`pagination_restore_success target=${targetPage} actual=${state.current}`});
      return;
    }catch(error){const message=error instanceof Error?error.message:String(error);emit({type:"pagination",stage:"pagination_restore_failed",target:targetPage,attempt,message:`pagination_restore_failed target=${targetPage} attempt=${attempt}/${maxAttempts} error=${message}`});if(attempt===maxAttempts)throw new Error(`PAGINATION_RESTORE_FAILED target=${targetPage} ${message}`);await page.waitForTimeout(250)}
  }
}
